"""
Seçilen 2 Santral için Excel Rapor Oluşturucu
Streamlit'te seçilen santraller için özel Excel raporu oluşturur
"""
...
class SelectedSantralExcel:
    def __init__(self):
        self.reports_dir = os.path.join(os.path.dirname(__file__), '..', 'reports', 'excel_reports')
        os.makedirs(self.reports_dir, exist_ok=True)
    
    def create_comparison_excel(self, santral1_name, santral1_data, santral2_name, santral2_data):
        """Seçilen 2 santral için karşılaştırmalı Excel raporu oluştur"""
        
        # Dosya adı
        safe_name1 = santral1_name.replace(' ', '_').replace('İ', 'I')
        safe_name2 = santral2_name.replace(' ', '_').replace('İ', 'I')
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        excel_path = os.path.join(
            self.reports_dir, 
            f'Karsilastirma_{safe_name1}_vs_{safe_name2}_{timestamp}.xlsx'
        )
        ...
        # Santral 1 Sheet'i
        self.create_santral_sheet(
            workbook, santral1_name, santral1_data, 'Santral_1',
            header_format, title_format, number_format, currency_format, date_format
        )
        ...
        self.create_comparison_sheet(
            ...
            header_format, title_format, number_format, currency_format
        )
        
        workbook.close()
        print(f"Karşılaştırmalı Excel raporu oluşturuldu: {excel_path}")
        return excel_path
    
    def create_santral_sheet(self, workbook, santral_name, df, sheet_name, 
                           header_format, title_format, number_format, currency_format, date_format):
        """Tek santral için sheet oluştur"""
        
        worksheet = workbook.add_worksheet(sheet_name)
        
        # Başlık
        worksheet.merge_range('A1:S1', santral_name, title_format)
        
        # Normal format - basit sütun başlıkları
        headers = [
    ...
    def create_comparison_sheet(self, workbook, santral1_name, santral1_data, 
                              santral2_name, santral2_data, 
                              header_format, title_format, number_format, currency_format):
        """Karşılaştırma sheet'i oluştur - İstenen formata uygun"""
        
        worksheet = workbook.add_worksheet('Karşılaştırma')
        
        # Santral 1 başlığı (A2 hücresinde)
        ...
        for col, header in enumerate(headers_row3):
            worksheet.write(2, col, header, header_format)  # 3. satır (0-indexed: 2)
        ...
        for i in range(12):  # 12 ay
            ay = i + 1
            row_idx = 3 + i  # 4. satırdan başla (0-indexed: 3)
        ...
        # Santral 1 Toplam satırı (16. satır)
        ...
        for col, header in enumerate(headers_row3):
            worksheet.write(18, col, header, header_format)  # 19. satır (0-indexed: 18)
        ...
        for i in range(12):  # 12 ay
            ay = i + 1
            row_idx = 19 + i  # 20. satırdan başla (0-indexed: 19)
        ...
        # Santral 2 Toplam satırı (32. satır)
        ...
        # Sütun genişliklerini ayarla
        worksheet.set_column('A:A', 8)   # Ay
        worksheet.set_column('B:C', 18)  # Üretim ve Dengesizlik
        worksheet.set_column('D:F', 15)  # Gelir sütunları
        worksheet.set_column('G:I', 18)  # Birim değerler
...
if __name__ == "__main__":
    ...
    if not santral1_data.empty and not santral2_data.empty:
        excel_gen = SelectedSantralExcel()
        excel_path = excel_gen.create_comparison_excel(
